...
interface MyUserApi {
    ...
    suspend fun getUsers(@Header("token") token: String): Response<ArrayList<MyUser>>

    @GET(Constants.GET_MY_USER)
    suspend fun getMyUser(@Header("token") token: String): Response<MyUser>
    ...
    suspend fun getDepartmentMembers(@Header("token") token: String, departmentId: Int): Response<ArrayList<MyUser>>
